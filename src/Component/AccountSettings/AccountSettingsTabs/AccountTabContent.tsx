...
// ========= Getting Actsetting Data
  ...
  const UpdateCryptoWallet = async (values: USDTwalletaddrespropsType) => {
    setspinner({ FormName: "cryptoWallet", Action: true });
    try {
      const { WalletAddress,  OTP } = values;
      const param = {
        OTP: OTP,
        ClientId:ClientID,
                ...
                <Card>
                  <CardBody>
                    <Row>
                      ...
                      {/* <Col sm="4" md="4" >
                        <Label>Recieving Address(Bep20(BSC)Chain)</Label>
                        <Field type="text" name="FXSTRecievingAddress" autoComplete="off" className="form-control" placeholder="Enter Recieving Address" />
                        <ErrorMessage name="FXSTRecievingAddress" component="div" className="text-danger" />
                      </Col> */}
                      <Col sm="4" md="6"  className="position-relative">
                        ...
                        <Btn color="info"
                          ...
                          disabled={disablebtn}
                          style={{ position: 'absolute', right: '15px', top: '45%', backgroundColor: '#fff', borderColor: '#fff', color: '#000' }}>
