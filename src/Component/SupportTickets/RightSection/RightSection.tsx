        ...
        <CardBody>
              <UL className="simple-list list-group ticket-block">
                {TicketListData.length > 0 ? TicketListData.map((itm:any, index:number)=><LI className='inbox-data  list-group-item border rounded p-2 mt-1' key={index}>
                    ...
                    <SvgIcon className='feather me-2' iconId='star'/><Image src={dynamicImage("avatar/user.png")} style={{width: '85px', height: '85px'}} alt="avatar" />
                    <span className='ms-2'>
                    ...
                    <Col md="3 text-end px-2">
                      <TicketModal btnName={"View More"} ModelName={"ViewMore"} ModelTitle={itm?.Title} onOpen={() => handleViewModal(itm?.QueryId)} modelData={modelData} refreshdata={setmodalDatareload} className={'custom-modal'}/>
                    </Col>
                    </Row>
                </LI>
                ) : <>
