...
import { UsdtBEPTitle, DepositFundTitle, SampleCards } from "../../../utils/Constant";
...
const USDTBEP20PageContainer = () => {
  const { getDepositWalletBalance, getBEP20Address, loading } = useDepositFundService();
  ...
  const GenerateTRC20Address = async () => {
    ...
    const res = await getBEP20Address(param);
  ...
  return (
    <>
      <Breadcrumbs mainTitle={UsdtBEPTitle} parent={DepositFundTitle} ChildName={UsdtBEPTitle} />
      <Container fluid>
        {loading && <Loader />}
        <Row>
          <Col xl="4">
